    ...
    <tbody>
      <tr v-for="(item, index) in data" :key="index">
        <td v-for="(header, index) in headers" :key="index" class="
          ...
          <p v-else>{{ (item[header.value]) ? (item[header.value].length > 20) ? item[header.value].substring(0, 20) +
            '...'
            : item[header.value] : "N/A" }}</p>
        </td>
        <td v-if="!hideActions" class="
          ...
          <div v-if="showActions[index]" class="absolute top-10 z-10 w-[150px] mt-4 bg-white shadow-md">
            <ul>
              <li>
                <a class="block w-full px-4 py-2 text-left text-blue-700 hover:bg-gray-100" v-if="allowedActions.details"
                  href="#!" @click="detailMethod(index)">Details</a>
              </li>
              <slot name="actions" :index="index"></slot>
              <li>
                <a class="block w-full px-4 py-2 text-left text-blue-700 hover:bg-gray-100" v-if="allowedActions.edit"
                  href="#!" @click="editItem(index)">Edit</a>
              </li>
              <li>
                <a class="block w-full px-4 py-2 text-left text-red-700 hover:bg-gray-100" v-if="allowedActions.delete"
                  href="#!" @click="deleteItem(index)">Delete</a>
              </li>
            </ul>
          </div>
        </td>
